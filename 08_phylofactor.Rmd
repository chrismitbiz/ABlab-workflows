...
factor.map = data.frame(Phylofactor = 1:PF$nfactors, group = rep(1, PF$nfactors))
# Then get tip labels of selected factors and groups and use as index to the taxonomy to get genus name
# Factor 1
taxon1 <-data.frame(tax_table(ps.phylo))[PF$tree$tip.label[PF$groups[[factor.map[1,1]]][[factor.map[1, 2]]]],]$Genus 
# Factor 2
taxon2 <- data.frame(tax_table(ps.phylo))[PF$tree$tip.label[PF$groups[[factor.map[2,1]]][[factor.map[2, 2]]]],]$Family
# Factor 4
taxon4 <- data.frame(tax_table(ps.phylo))[PF$tree$tip.label[PF$groups[[factor.map[4,1]]][[factor.map[4, 2]]]],]$Family
# Factor 5
taxon5 <- data.frame(tax_table(ps.phylo))[PF$tree$tip.label[PF$groups[[factor.map[5,1]]][[factor.map[5, 2]]]],]$Genus
...
# add node labels to the nodes
p + geom_cladelabel(node = 
                 (factor.map %>% dplyr::filter(Phylofactor ==  1))$node, 
                 label = paste('italic(', taxon1,')'), 
                 offset.text = 25, 
                 hjust = "center",
                 fontsize = 3,
                 parse = TRUE) + 
   geom_cladelabel(node = 
                 (factor.map %>% dplyr::filter(Phylofactor ==  2))$node, 
                 label = paste('italic(', taxon2,')'), 
                 offset.text = 10, 
                 hjust = "center",
                 fontsize = 3,
                 parse = TRUE) + 
   geom_cladelabel(node = 
                 ...
                 label = paste('italic(', taxon4,')'), 
                 offset.text = 10, 
                 hjust = "center",
                 fontsize = 3,
                 parse = TRUE) + 
   geom_cladelabel(node = 
                 (factor.map %>% dplyr::filter(Phylofactor ==  5))$node, 
                 label = paste('italic(', taxon5,')'), 
                 offset.text = 20, 
                 hjust = "center",
                 fontsize = 3,
                 parse = TRUE)
