...
nd <- NULL
for (i in 1:m) {
    grp <- PF$tree$tip.label[PF$groups[[factor.map[3,1]]][[factor.map[3, 2]]]]
...
# Create a df of taxa names for annotation
...
for (i in 1:nrow(tax.names)) {
p <- p + geom_cladelabel(node = 
                 (factor.map %>% dplyr::filter(Phylofactor ==  i))$node, 
                 label = paste('italic(', tax.names[i,],')'), 
                 offset.text = 25, 
                 hjust = "center",
                 fontsize = 3,
                 parse = TRUE)
