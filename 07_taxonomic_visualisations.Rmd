...
```{r 07metadatacheck, include=FALSE, echo=TRUE, warning=FALSE, message=FALSE}
...
# Some arbitrary quality filtering of individual ASVs. For example, here we choose to
# remove ASVs that have very few reads (i.e. less than 5 counts of the amplicon sequence).
# The may remove some more arbitrary ASVs. 
...
# rarefy and create a new phyloseq object containing normalised abundances
ps_rare <- phyloseq::rarefy_even_depth(ps_flt, 
    sample.size = min(colSums(otu_table(ps_flt))),  
    rngseed = TRUE, # set a seed for the random subsampling
    replace = FALSE, # without replacement
    trimOTUs = TRUE ) # remove ASVs that are no longer observed in any sample 
# ps_rare 
# phyloseq-class experiment-level object
# otu_table()   OTU Table:         [ 3862 taxa and 51 samples ]
# sample_data() Sample Data:       [ 51 samples by 15 sample variables ]
#tax_table()   Taxonomy Table:    [ 3862 taxa by 7 taxonomic ranks ]
# phy_tree()    Phylogenetic Tree: [ 3862 tips and 3861 internal nodes ]
```
...
```{r Phylumplots, echo=TRUE, message=FALSE, warning=FALSE, results=FALSE, fig.asp=0.45, fig.width=13}
# Phylum plot  
