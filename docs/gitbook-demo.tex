...
Furthermore, beta diversity, i.e.~four different type of ordinations are plotted. The examples, include an non-metric dimensional scaling on Bray-Curtis dissimilaries, Principal Component Analysis (Aitchison distance), Redundancy Analysis ( Aitchison) and a Principal Coordinate Analysis on unweighted uniFrac distances.
...
There are 15 columns with some character (chr), factor or integer (int) variables.
...
\hypertarget{pre-processing-of-phyloseq-object}{%
\subsection{Pre-processing of phyloseq object}\label{pre-processing-of-phyloseq-object}}
...
\CommentTok{\# sample\_data() Sample Data:       [ 51 samples by 14 sample variables ]}
\CommentTok{\# tax\_table()   Taxonomy Table:    [ 3853 taxa by 7 taxonomic ranks ]}
\CommentTok{\# phy\_tree()    Phylogenetic Tree: [ 3853 tips and 3852 internal nodes ]}
...
\CommentTok{\# phyloseq{-}class experiment{-}level object}
\CommentTok{\# otu\_table()   OTU Table:         [ 3790 taxa and 51 samples ]}
\CommentTok{\# sample\_data() Sample Data:       [ 51 samples by 14 sample variables ]}
\CommentTok{\# tax\_table()   Taxonomy Table:    [ 3790 taxa by 7 taxonomic ranks ]}
\CommentTok{\# phy\_tree()    Phylogenetic Tree: [ 3790 tips and 3789 internal nodes ]}
...
\CommentTok{\# this replaced the old metadata with new metadata that includes the diversity indices for this phyloseq object }
...
##  $ Sample.Name    : chr  "16SAMP-17236" "16SAMP-17231" "16SAMP-17233" "MQ170915-14" ...
##  $ Location       : Factor w/ 12 levels "Avedoere","Damhusaaen",..: 4 11 5 12 12 12 12 2 11 11 ...
##  $ Observed       : num  160 349 250 261 387 451 400 301 278 371 ...
##  $ Chao1          : num  160 353 251 261 393 ...
##  $ se.chao1       : num  0.249 3.33 1.256 0.621 3.791 ...
##  $ ACE            : num  160 352 251 261 392 ...
##  $ se.ACE         : num  4.42 9.13 7.49 7.5 9.79 ...
##  $ Shannon        : num  4.23 4.86 4.72 4.79 5.03 ...
##  $ Simpson        : num  0.966 0.975 0.982 0.983 0.986 ...
##  $ InvSimpson     : num  29.2 40.1 55.1 58.6 71.3 ...
##  $ Fisher         : num  27.2 70.7 46.8 49.3 80.5 ...
##  $ pielou_ev      : num  0.834 0.83 0.855 0.861 0.844 ...
\end{verbatim}
...
\NormalTok{cols }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"\#F9E79F"}\NormalTok{, }\StringTok{"\#FDEBD0"}\NormalTok{, }\StringTok{"\#F6DDCC"}\NormalTok{,  }\StringTok{"\#E5E8E8"}\NormalTok{, }\StringTok{"\#CCD1D1"}\NormalTok{, }\StringTok{"\#AAB7B8"}\NormalTok{, }\StringTok{"\#707B7C"}\NormalTok{, }\StringTok{"\#566573"}\NormalTok{,}\StringTok{"\#82E0AA"}\NormalTok{, }\StringTok{"\#641E16"}\NormalTok{, }\StringTok{"\#8E44AD"}\NormalTok{, }\StringTok{"\#E74C3C"}\NormalTok{)}

\CommentTok{\# you can also name each colour according to your treatment. This will help ggplot to associated the same colour for each treatment across different ggplots. For example, if we want to show different colour by location in this dataset: }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{physeqNMDS }\OtherTok{\textless{}{-}}\NormalTok{  ps.flt  }\CommentTok{\# just to keep it separate.}
...
\NormalTok{                                      ordination , }
     ...
     \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{size =}\NormalTok{ pielou\_ev), }\AttributeTok{alpha =} \FloatTok{0.75}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# shape size reflective of Pielou, also make transparent }
...
\NormalTok{RDAplot }\OtherTok{\textless{}{-}}\NormalTok{ phyloseq}\SpecialCharTok{::}\FunctionTok{plot\_ordination}\NormalTok{(physeqPCA, ordination , }
  ...
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept=}\DecValTok{0}\NormalTok{, }\AttributeTok{linetype=}\StringTok{"dashed"}\NormalTok{, }\AttributeTok{color =} \StringTok{"gray"}\NormalTok{) }\SpecialCharTok{+}    \CommentTok{\# shape size reflective of Pielou  }
     \FunctionTok{annotate}\NormalTok{(}\AttributeTok{geom =} \StringTok{\textquotesingle{}text\textquotesingle{}}\NormalTok{, }\AttributeTok{label =} \FunctionTok{paste}\NormalTok{(}\StringTok{"Aitchison distances"}\NormalTok{), }
             \AttributeTok{x =} \SpecialCharTok{{-}}\ConstantTok{Inf}\NormalTok{, }\AttributeTok{y =} \SpecialCharTok{{-}}\ConstantTok{Inf}\NormalTok{, }\AttributeTok{hjust =} \SpecialCharTok{{-}}\FloatTok{0.05}\NormalTok{, }\AttributeTok{vjust =} \SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{size =}\NormalTok{ pielou\_ev)) }\SpecialCharTok{+}        \CommentTok{\# shape size reflective of Pielou Evenness}
  \CommentTok{\#scale\_colour\_gradient(low = colourlow, high = colourhigh) +}
  \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"RDA Abundances (clr) \textasciitilde{} Treatment "}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values =}\NormalTok{ cols)}
