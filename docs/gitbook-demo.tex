...
Various data normalisation methods exists and opinions on their applicability diverge.
...
Rarefying data is one of the more contested methods. We acknowledge that \textbf{rarefying data} (i.e.~the random subsampling of abundances to usually the smallest library size) results in loss of data and statistical power and is thus \textbf{not recommended for differential abundance analyses}. Check out this well cited paper here to learn why: \href{https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1003531}{Waste Not, Want Not: Why Rarefying Microbiome Data Is Inadmissible}

However, rarefying still ``\emph{outperformed all the other normalization methods for producing accurate BC (Bray-Curtis) dissimilarities and subsequent PCoAs and PERMANOVAs.}'' Paper here: \href{https://doi.org/10.1111/2041-210X.13115}{Methods for normalizing microbiome data: An ecological perspective}.

For our workflows we may \textbf{rarefy data} before creating \textbf{alpha diversity} indices (Shannon, Simpsons etc), \textbf{beta diversity} indices (Bray-Curtis dissimilarities).

Other types of normalisation methods should be considered for differential abundance analyses. My preference is to use \textbf{centred-log ratios} (Aitchison distances), which makes count normalisation unnecessary. The \textbf{compositional approach} and centred log-ratios are further discussed in the paper \href{https://doi.org/10.3389/fmicb.2017.02224}{Microbiome Datasets Are Compositional: And This Is Not Optional}.

\hypertarget{workflows-from-other-lab-groups}{%
\section{Workflows from other lab groups}\label{workflows-from-other-lab-groups}}

The following review ``\emph{should serve as a starting point for considering what options are available}'': \href{https://www.frontiersin.org/articles/10.3389/fmicb.2017.01561/full}{Analysing microbial community composition through amplicon sequencing: from sampling to hypothesis testing}
...
As mentioned in Chapter \ref{gettingstarted}, prior to establishing alpha diversity indices, as well as Bray-Curtis dissimilarities, we will stick to rarefying sequences to a specific library size (library size = sum of sequence counts of a sample) to normalise the data and prevent bias due to library size. Usually, this is done using the smallest library size of all samples. If you want to learn more about rarefying - this paper explains the process: \href{https://www.nature.com/articles/s41598-021-01636-1}{Enhancing diversity analysis by repeatedly rarefying next generation sequencing data describing microbial communities}.\\
We will follow the recommendations from the authors of this paper to resample without replacement.

Note that rarefying is generally not recommended for differential abundance analyses due to the loss of statistical power. Learn why here: \href{https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1003531}{Waste Not, Want Not: Why Rarefying Microbiome Data Is Inadmissible}.
...
My preference is to use centred-log ratios (Aitchison distances), which makes count normalisation unnecessary. Aitchison distances are included into the workflow below in a PCA and RDA. The compositional approach and centred log-ratios are further discussed in the paper \href{https://doi.org/10.3389/fmicb.2017.02224}{Microbiome Datasets Are Compositional: And This Is Not Optional}.\\
...
\tightlist
\item
  A phyloseq object; either read in from a pre-saved \texttt{.rds} file or created as described in chapter \ref{Qiime2R} under ``Import qiime-files and create a phyloseq object''.
\end{itemize}
...
\NormalTok{ordination }\OtherTok{\textless{}{-}}\NormalTok{ phyloseq}\SpecialCharTok{::}\FunctionTok{ordinate}\NormalTok{(physeqNMDS  , }\StringTok{"NMDS"}\NormalTok{, }\AttributeTok{distance =} \StringTok{\textquotesingle{}bray\textquotesingle{}}\NormalTok{) }\CommentTok{\# using Bray{-}Curtis dissimilarities here. }
...
\NormalTok{                                      ordination , }
     ...
     \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{size =}\NormalTok{ pielou\_ev), }\AttributeTok{alpha =} \FloatTok{0.75}\NormalTok{) }\SpecialCharTok{+}   \CommentTok{\# shape size in proportion to Pielou Evenness, also make transparent }
...
Some ggplots to visualise and compare taxonomy.
...
Colors: Before you create ggplots to compare taxonomic compositions between samples and treatments it is helpful to create a named color vector that will apply to all your plots and ensures that each taxonomic name is a specific color across all your plots. It is only optional as ggplot can refer to its default colors, however the colours for individual taxa will be different between plots.
...
First create a vector of all possible taxonomic names that could be included in your ggplot. For example, if you plan to plot on phylum-level then all phyla names are extracted from the phyloseq object and then those names are given a color code.

\hypertarget{normalisation-and-pre-filtering}{%
\subsection{Normalisation and pre-filtering}\label{normalisation-and-pre-filtering}}

For a `fair' comparison, abundances should be normalised to minimise potential bias due to the different sample library sizes, i.e the different total number of sequences in each sample. See alsoChapter \ref{plottingdiversity} in subchapter ``Pre-filter, rarefy and calculate diversity''.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Some arbitrary quality filtering of individual ASVs. For example, here we choose to}
\CommentTok{\# remove ASVs that have very few reads (i.e. less than 5 counts of the amplicon sequence).}
\CommentTok{\# The may remove some more arbitrary ASVs. }
...
\CommentTok{\# rarefy and create a new phyloseq object containing normalised abundances}
\NormalTok{ps\_rare }\OtherTok{\textless{}{-}}\NormalTok{ phyloseq}\SpecialCharTok{::}\FunctionTok{rarefy\_even\_depth}\NormalTok{(ps\_flt, }
    \AttributeTok{sample.size =} \FunctionTok{min}\NormalTok{(}\FunctionTok{colSums}\NormalTok{(}\FunctionTok{otu\_table}\NormalTok{(ps\_flt))),  }
    \AttributeTok{rngseed =} \ConstantTok{TRUE}\NormalTok{, }\CommentTok{\# set a seed for the random subsampling}
    \AttributeTok{replace =} \ConstantTok{FALSE}\NormalTok{, }\CommentTok{\# without replacement}
    \AttributeTok{trimOTUs =} \ConstantTok{TRUE}\NormalTok{ ) }\CommentTok{\# remove ASVs that are no longer observed in any sample }
\CommentTok{\# ps\_rare }
\CommentTok{\# phyloseq{-}class experiment{-}level object}
\CommentTok{\# otu\_table()   OTU Table:         [ 3862 taxa and 51 samples ]}
\CommentTok{\# sample\_data() Sample Data:       [ 51 samples by 15 sample variables ]}
\CommentTok{\#tax\_table()   Taxonomy Table:    [ 3862 taxa by 7 taxonomic ranks ]}
\CommentTok{\# phy\_tree()    Phylogenetic Tree: [ 3862 tips and 3861 internal nodes ]}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{order }\OtherTok{\textless{}{-}}\NormalTok{ taxa\_data}\SpecialCharTok{$}\NormalTok{Phylum }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ unique}
...
\hypertarget{prevelance-table}{%
\subsection{Prevelance table}\label{prevelance-table}}

A prevalence table may assist in getting an overview of the abundances.
